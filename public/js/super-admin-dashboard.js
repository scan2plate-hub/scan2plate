...
function renderSummary() {
  ...
  const cards = [["Total Restaurants",restaurants.length,"fa-store","totalRestaurants"],["Active Restaurants",active.length,"fa-circle-check","activeRestaurants"],["Suspended / Expired",inactive.length,"fa-triangle-exclamation","inactiveRestaurants"],["Monthly Revenue",fmtCurrency(monthlyRevenue),"fa-indian-rupee-sign","monthlyRevenue"],["Basic Plans",counts.basic,"fa-seedling","basicPlans"],["Advance Plans",counts.advance,"fa-bolt","advancePlans"],["Trial Restaurants",restaurants.filter(restaurant => String(restaurant.plan || "").toLowerCase() === "trial").length,"fa-hourglass-half","trialRestaurants"],["Total Orders",totalOrders,"fa-receipt","totalOrders"]];
  summaryCards.innerHTML = cards.map(([label,value,icon,id]) => `<div class="sa-stat"><div class="sa-stat-top"><span>${label}</span><span class="sa-stat-icon"><i class="fa-solid ${icon}"></i></span></div><b id="${id}">${value}</b><small>Across all restaurants</small></div>`).join("");
}
...
function renderRestaurants() {
  ...
  restaurantRows.innerHTML = list.length ? list.map(restaurant => { const stats = orderStats(restaurant.id); const plan = normalizePlan(restaurant.plan); const status = effectiveStatus(restaurant); const adminAction = status === "active" ? `<button class="sa-btn ghost" data-action="admin" data-id="${escapeHtml(restaurant.id)}">Admin</button>` : `<button class="sa-btn ghost" data-action="renew" data-id="${escapeHtml(restaurant.id)}">Expired</button>`; return `<tr><td><span class="sa-restaurant">${escapeHtml(restaurant.name || restaurant.id)}</span><span class="sa-id">${escapeHtml(restaurant.id)}</span></td><td>${escapeHtml(restaurant.ownerName || "—")}<span class="sa-sub">${escapeHtml(restaurant.adminEmail || restaurant.email || "—")}</span></td><td><span class="sa-badge ${plan}">${escapeHtml(plan)}</span><span class="sa-sub">${fmtCurrency(planPrices()[plan] || 0)}/mo</span></td><td><span class="sa-badge ${status}">${escapeHtml(status)}</span></td><td>${dateLabel(restaurant.createdAt)}<span class="sa-sub">Expires: ${dateLabel(restaurant.expiryDate)}</span></td><td>${stats.count}</td><td>${fmtCurrency(stats.revenue)}</td><td><div class="sa-actions"><button class="sa-btn ghost" data-action="view" data-id="${escapeHtml(restaurant.id)}">View</button><button class="sa-btn ghost" data-action="plan" data-id="${escapeHtml(restaurant.id)}">Edit / Plan</button><button class="sa-btn ghost" data-action="activate" data-id="${escapeHtml(restaurant.id)}">Activate</button><button class="sa-btn danger" data-action="suspend" data-id="${escapeHtml(restaurant.id)}">Suspend</button><button class="sa-btn ghost" data-action="renew" data-id="${escapeHtml(restaurant.id)}">Renew</button>${adminAction}<button class="sa-btn ghost" data-action="qr" data-id="${escapeHtml(restaurant.id)}">QR</button></div></td></tr>`; }).join("") : `<tr><td colspan="8"><div class="sa-empty"><i class="fa-solid fa-magnifying-glass"></i>No restaurants found.</div></td></tr>`;
...
async function handleRestaurantAction(action, id) { const restaurant = restaurants.find(item => item.id === id); if (!restaurant) return; if (action === "view" || action === "plan") { modal.classList.add("open"); modalBody.innerHTML = `<div class="sa-form-grid"><div><strong>${escapeHtml(restaurant.name || id)}</strong><p class="sa-sub">${escapeHtml(id)} · ${escapeHtml(restaurant.adminEmail || restaurant.email || "No email")}</p><p>Owner: ${escapeHtml(restaurant.ownerName || "—")}</p><p>Orders: ${orderStats(id).count} · Revenue: ${fmtCurrency(orderStats(id).revenue)}</p></div><div><label class="sa-sub">Change plan</label><select id="modalPlan" class="sa-select" style="width:100%"><option value="basic">Basic</option><option value="advance">Advance</option><option value="enterprise">Enterprise</option></select><button id="saveModalPlan" class="sa-btn" style="margin-top:10px">Save plan</button></div></div>`; $("#modalPlan").value = normalizePlan(restaurant.plan); $("#saveModalPlan").onclick = () => updateRestaurant(id,{plan:$("#modalPlan").value}); return; } if (action === "admin") { window.open("./admin-dashboard.html", "_blank", "noopener"); return; } if (action === "qr") { $("#qrRestaurant").value=id; switchSection("qr"); return; } if (action === "renew") { openRenewalModal(restaurant); return; } if (action === "activate") return updateRestaurant(id,{status:"active",subscriptionStatus:"active"}); if (action === "suspend" && confirm(`Suspend ${restaurant.name || id}?`)) return updateRestaurant(id,{status:"suspended",subscriptionStatus:"suspended"}); }
...
function applyBusinessTerminology() {
  document.querySelectorAll("h1,h2,a,button,p,span").forEach(el => {
    if (el.children.length) return;
    const map = { "Restaurants":"Businesses", "Restaurant":"Business", "Add Restaurant":"Add Business", "Create restaurant":"Create business", "Restaurant Onboarding Sheet":"Business Onboarding Sheet", "Recent restaurants":"Recent businesses" };
    if (map[el.textContent.trim()]) el.textContent = map[el.textContent.trim()];
  });
  const header = restaurantRows?.closest("table")?.querySelector("thead");
  if (header) header.innerHTML = "<tr><th>Business Name</th><th>Business Type</th><th>Business ID</th><th>Owner Phone</th><th>Plan</th><th>Expiry Date</th><th>Status</th><th>Panel Type</th><th>Actions</th></tr>";
}
function renderRestaurants() {
  const list = filteredRestaurants();
  restaurantRows.innerHTML = list.length ? list.map(business => { const plan=normalizePlan(business.plan), status=effectiveStatus(business), type=businessTypeOf(business), panel=panelTypeOf(business); return `<tr><td><span class="sa-restaurant">${escapeHtml(business.name || business.restaurantName || business.id)}</span></td><td>${escapeHtml(type)}</td><td><span class="sa-id">${escapeHtml(business.id)}</span></td><td>${escapeHtml(business.phone || business.ownerName || "—")}</td><td><span class="sa-badge ${plan}">${escapeHtml(plan)}</span></td><td>${dateLabel(business.expiryDate || business.planExpiryDate)}</td><td><span class="sa-badge ${status}">${escapeHtml(status)}</span></td><td>${escapeHtml(panel)}</td><td><div class="sa-actions"><button class="sa-btn ghost" data-action="view" data-id="${escapeHtml(business.id)}">View</button><button class="sa-btn ghost" data-action="plan" data-id="${escapeHtml(business.id)}">Edit / Plan</button><button class="sa-btn ghost" data-action="qr" data-id="${escapeHtml(business.id)}">QR</button><button class="sa-btn ghost" data-action="renew" data-id="${escapeHtml(business.id)}">Renew</button></div></td></tr>`; }).join("") : `<tr><td colspan="9"><div class="sa-empty">No businesses found.</div></td></tr>`;
  restaurantRows.querySelectorAll("[data-action]").forEach(button => button.addEventListener("click", () => handleRestaurantAction(button.dataset.action, button.dataset.id)));
}
...
async function loadData() { try { const [restaurantSnap,orderSnap] = await Promise.all([getDocs(collection(db,"restaurants")),getDocs(collection(db,"orders"))]); restaurants=restaurantSnap.docs.map(snapshot=>{ const data=snapshot.data(); return { id:snapshot.id, ...data, businessType:data.businessType || data.restaurantType || "Restaurant", orderMode:data.orderMode || "table", panelType:data.panelType || "RestaurantAdmin" }; }).sort((a,b)=>(dateFrom(b.createdAt)?.getTime()||0)-(dateFrom(a.createdAt)?.getTime()||0)); orders=orderSnap.docs.map(snapshot=>({id:snapshot.id,...snapshot.data()})); await Promise.all(restaurants.map(async restaurant => { try { restaurant.menuItemCount=(await getDocs(collection(db,"restaurants",restaurant.id,"menu"))).size; } catch { restaurant.menuItemCount=null; } })); renderSummary();renderAlerts();renderRecent();renderRestaurants();renderOnboarding();renderPlans();renderReports();populateQrRestaurants(); } catch(error) { console.error("Super admin data load failed",error); [recentRestaurants,restaurantRows,onboardingRows].filter(Boolean).forEach(element=>element.innerHTML=`<div class="sa-empty">Unable to load data. Please refresh.</div>`); } }
