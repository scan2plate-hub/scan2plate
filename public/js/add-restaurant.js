...
saveRestaurantBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    // mapping them explicitly cannot affect an existing business. A hostel is
    // room-based like a hotel; a salon takes counter/appointment orders, not
    // table QR orders.
    const orderMode = ["cafe", "street vendor", "food court", "bakery", "sweet shop", "dhaba", "fast food", "juice shop", "tea stall", "salon"].includes(type) ? "token" : ["hotel", "hostel"].includes(type) ? "room" : type === "cloud kitchen" ? "delivery" : "table";
