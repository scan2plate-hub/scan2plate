...
setTimeout(() => {
  ...
  if(date&&!document.getElementById("bill-all-dates")){date.insertAdjacentHTML("afterend",'<button id="bill-all-dates" class="btn btn-outline btn-sm" type="button">All Dates</button>');document.getElementById("bill-all-dates")?.addEventListener("click",()=>{date.dataset.touched="1";date.value="";resetWindow(document.getElementById("bill-list"));renderBills()});}
...
document.getElementById("st-delete-modal").addEventListener("click",event=>{if(event.target.id==="st-delete-modal")closeStaffDeleteConfirm();});document.getElementById("adv-save").onclick=addAdvance;document.getElementById("del-save").onclick=saveIntegration;const rerenderBillsFromFilters=()=>{resetWindow(document.getElementById("bill-list"));renderBills();};
const debouncedRenderBills=debounce(rerenderBillsFromFilters,180);["bill-q","bill-date","bill-status","bill-method","bill-sort"].forEach(id=>{const el=document.getElementById(id);if(el)el.oninput=debouncedRenderBills;});document.getElementById("at-date").onchange=renderStaff;document.getElementById("pay-month").onchange=renderPayroll;document.getElementById("pay-print").onclick=()=>print("pay-table","Salary Slip / Payroll");document.getElementById("acc-print").onclick=()=>print("acc-table","Accounts Report");document.getElementById("acc-csv").onclick=()=>exportCsv([["Report","Generated"],["Accounts",today()]],"accounts-report.csv");
